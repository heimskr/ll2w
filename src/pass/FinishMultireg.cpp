	...
	void finishMultireg(Function &function) {
		...
		for (const InstructionPtr &instruction: function.linearInstructions) {
			if (auto *defsource = dynamic_cast<DeferredSourceMoveInstruction *>(instruction.get())) {
				...
				auto move = std::make_shared<MoveInstruction>(std::move(var), defsource->rd);
				function.insertBefore(instruction, std::move(move))->setDebug(defsource)->extract();
				to_remove.push_back(instruction);
				continue;
			}

			if (auto *defdest = dynamic_cast<DeferredDestinationMoveInstruction *>(instruction.get())) {
			...
			if (auto *load = dynamic_cast<LoadRInstruction *>(instruction.get())) {
				...
				while (8 <= bytes_remaining) {
					...
					auto new_load = std::make_shared<LoadRInstruction>(m4, precolored, WASMSize::Word);
					function.insertBefore(instruction, new_load)->setDebug(load)->extract();
					bytes_remaining -= 8;
					if (0 < bytes_remaining) {
						function.insertBefore(instruction, std::make_shared<AddIInstruction>(m4, 8, m4))->setDebug(load)->extract();
					}
				}
				...
				for (int size = 4; 0 < size; size /= 2) {
					if (size <= bytes_remaining) {
						auto precolored = function.makePrecoloredVariable(*iter++, load->parent.lock());
						auto new_load = std::make_shared<LoadRInstruction>(m4, precolored, static_cast<WASMSize>(size));
						function.insertBefore(instruction, new_load)->setDebug(load)->extract();
						bytes_remaining -= size;
						if (0 < bytes_remaining) {
							function.insertBefore(instruction, std::make_shared<AddIInstruction>(m4, size, m4))->setDebug(load)->extract();
						}
					}
				}
			...
			if (auto *store = dynamic_cast<StoreRInstruction *>(instruction.get())) {
				...
				while (8 <= bytes_remaining) {
					...
					bytes_remaining -= 8;
					if (0 < bytes_remaining) {
						function.insertBefore(instruction, std::make_shared<AddIInstruction>(m4, 8, m4))->setDebug(store)->extract();
					}
				}
				...
				for (int size = 4; 0 < size; size /= 2) {
					if (size <= bytes_remaining) {
						auto precolored = function.makePrecoloredVariable(*iter++, store->parent.lock());
						auto new_store = std::make_shared<StoreRInstruction>(precolored, m4, static_cast<WASMSize>(size));
						function.insertBefore(instruction, new_store)->setDebug(store)->extract();
						bytes_remaining -= size;
						if (0 < bytes_remaining) {
							function.insertBefore(instruction, std::make_shared<AddIInstruction>(m4, size, m4))->setDebug(store)->extract();
						}
					}
				}
